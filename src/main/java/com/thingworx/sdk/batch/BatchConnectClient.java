...
public class BatchConnectClient extends ConnectedThingClient {
    ...
    public BatchConnectClient(ClientConfigurator config) throws Exception {
        super(config);
    }
...
//        options.addOption("c", true, "Count, number of Steam sensors to create.");
        ...
        if (args.length == 0 || !cmd.hasOption("h")|| !cmd.hasOption("k")) {
            ...
            formatter.printHelp("BatchConnect", options);
            System.exit(-1);
        }
        ...
        if(cmd.hasOption("j")) {
            ...
            if(cmd.hasOption("q")) {
            	config.setProxyPassCallback( new BatchPasswordCallback(cmd.getOptionValue("q")) );
            }
        }
        ...
        config.setSecurityClaims( new BatchPasswordCallback( cmd.getOptionValue("k") ) );
...
//        }
        ...
        BatchConnectClient client = new BatchConnectClient(config);

        String thingBaseName = "BatchAPI";
        if(cmd.hasOption("t")) {
...
//                thingName = thingBaseName;
            final BatchConnectThing batchThing =
                    new BatchConnectThing(thingBaseName, "Batch API Connect", null, client,fileLogAppender);
            client.bindThing(batchThing);

            batchThing.addPropertyChangeListener(new VirtualThingPropertyChangeListener() {
                @Override
                public void propertyChangeEventReceived(VirtualThingPropertyChangeEvent evt) {
                    if ("TemperatureLimit".equals(evt.getPropertyDefinition().getName())) {
                        System.out.println(String.format("Temperature limit on %s has been changed to %s°.", batchThing.getName(),
